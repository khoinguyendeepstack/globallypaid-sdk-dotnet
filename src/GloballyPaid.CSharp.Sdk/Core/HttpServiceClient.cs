...
namespace GloballyPaid
{
    public partial class HttpServiceClient : IDisposable
    {
        ...
        {
            GetResponseMessageAsync(GetDeleteAction(resourceUri)).GetAwaiter().GetResult();
        }
        ...
        private void InitializeHttpClient()
        {
            ...
            if (!string.IsNullOrEmpty(Configuration.AuthenticationScheme))
            {
                ...
                {
                    throw new GloballyPaidException("Your Globally Paid Token API Key is not configured");
                }
        ...
        {
            return x => x.PostAsync(resourceUri, new StringContent(JsonConvert.SerializeObject(data), Encoding.UTF8, GloballyPaidConstants.ApplicationJson), cancellationToken);
        }
        ...
        {
            return x => x.PutAsync(resourceUri, new StringContent(JsonConvert.SerializeObject(data), Encoding.UTF8, GloballyPaidConstants.ApplicationJson), cancellationToken);
        }
        ...
        private Func<HttpClient, Task<HttpResponseMessage>> GetPatchAction<TData>(string resourceUri, TData data, CancellationToken cancellationToken = default)
        {
            ...
            {
                Content = new StringContent(JsonConvert.SerializeObject(data), Encoding.UTF8, GloballyPaidConstants.ApplicationJson)
            }, cancellationToken);
        }
        ...
        private Func<HttpClient, Task<HttpResponseMessage>> GetDeleteAction(string resourceUri,  CancellationToken cancellationToken = default)
        {
            ...
            {
                Content = new StringContent(JsonConvert.SerializeObject(string.Empty), Encoding.UTF8, GloballyPaidConstants.ApplicationJson)
            }, cancellationToken);
        }
